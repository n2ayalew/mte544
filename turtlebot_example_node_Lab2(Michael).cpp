...
float realHeight = 15; //was 15
float targetResolution = 0.01;  // was 0.02
int gridSize = realHeight/targetResolution;
...
// shitty measurement model --> but it will do :D
 float measModel(int length, int i){
    if (length - i > 2)  // sets last 2 points
        return 0.35;  // flipped these twoa around
    else
        return 0.7; // flipped these two around
 }
...
int main(int argc, char **argv) {
    ...
    //Set the loop rate
    ros::Rate loop_rate(8); //10 update rate --> fast enough to keep up with scans  // try 7 after
    ...
    //pathPose.pose.orientation = ips_yaw;
